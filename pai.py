...
async def generate_image(event):
    ...
    try:
        ...
        if len(f"Image Genrated Successfully\nPrompt: {args}") <= 1024:
            await event.client.send_file(
                event.chat_id,
                image_path,
                caption=f"<b>Image Genrated Successfully</b>\n<b>Prompt:</b> <code>{args}</code>\n\n<blockquote>©️ @RemainAlways</blockquote>",
