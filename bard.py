...
API_URL = "https://widipe.com/bard"
API_GEM = "https://widipe.com/gemini"

@ultroid_cmd(pattern="bard(?:\s|$)([\s\S]*)")
        ...
        if response.status_code == 200:
            ...
            if 'result' in data:
                ...
                if len(result_text) > 4095:
                    with open("bard.txt", "w", encoding="utf-8") as file:
...
@ultroid_cmd(pattern="gemini(?:\s|$)([\s\S]*)")
async def gpt4_query(event):
    msg = await event.eor("💫")
    if event.reply_to_msg_id:
        reply = await event.get_reply_message()
    ...
    try:
        # Sending the request to the API
        response = requests.get(API_GEM, headers=headers, params=params)
        if response.status_code == 200:
            data = response.json()
            if 'result' in data:
                result_text = data['result']
